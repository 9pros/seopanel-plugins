...
class SBSettings extends SocialBookmarker{
	...
	function __getAllSettings() {
		...
		$sql = "select * from sb_settings order by id";
	...
	function updatePluginSettings($postInfo) {
		...
		foreach($setList as $setInfo){
			...
			$sql = "update sb_settings set set_val='".addslashes($postInfo[$setInfo['set_name']])."' where set_name='{$setInfo['set_name']}'";
			$this->db->query($sql);
		}
