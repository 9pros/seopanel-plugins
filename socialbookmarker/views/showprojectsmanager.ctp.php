...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
	<tr class="listHead">
		...
		<td><?=$spText['label']['Title']?></td>
		<td><?=$spText['common']['Website']?></td>
		<td><?=$pluginText['Last Shared']?></td>
	...
	if(count($list) > 0){
		...
		foreach($list as $i => $listInfo){
            ...
            $projectLink = scriptAJAXLinkHref(PLUGIN_SCRIPT_URL, 'content', "action=editproject&project_id={$listInfo['id']}", "{$listInfo['share_title']}")
			?>
			<tr class="<?=$class?>">
				...
				<td class="td_br_right left"><?=$listInfo['last_shared_name']?></td>
				...
				<td class="<?=$rightBotClass?>" width="100px">
					...
					?>
					<select style="width: 124px;" name="action" id="action<?=$listInfo['id']?>" onchange="doPluginAction('<?=$pgScriptPath?>', 'content', 'project_id=<?=$listInfo['id']?>&pageno=<?=$pageNo?>&showhead=1', <?=$listInfo['id']?>)">
						<option value="select">-- <?=$spText['common']['Select']?> --</option>
						<?php if ($listInfo['status']) {?>
    						<option value="runproject"><?=$pluginText['Run Project']?></option>
    					<?php }?>
						<option value="<?=$statAction?>"><?=$statLabel?></option>
						<option value="editproject"><?=$spText['common']['Edit']?></option>
						<option value="deleteproject"><?=$spText['common']['Delete']?></option>
					</select>
				</td>
			</tr>
			<?php
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="actionSec">
	<tr>
    	<td style="padding-top: 6px;">
         	<a onclick="<?=pluginGETMethod('action=newproject&website_id='.$websiteId, 'content')?>" href="javascript:void(0);" class="actionbut">
         		<?=$pluginText['New Project']?>
         	</a>
    	</td>
	</tr>
</table>