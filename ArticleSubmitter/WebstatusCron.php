...
if (!file_exists($spLoadFile)) {
    $dirpath = str_ireplace('/plugins/ArticleSubmitter/WebstatusCrono.php', '', $_SERVER['SCRIPT_FILENAME']);
    $spLoadFile = $dirpath."/includes/sp-load.php";
...
if ($spLoadFileExist) {
    ...
    if (!empty($_SERVER['REQUEST_METHOD'])) {
        ...
        $pluginInfo = $controller->__getSeoPluginInfo('ArticleSubmitter', 'name');
        $info['pid'] = $pluginInfo['id'];
        $info['action'] = "WebstatusCronjob";
        $_GET['doc_type'] = 'export';
        $controller->manageSeoPlugins($info);
