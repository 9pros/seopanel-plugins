...
if (! file_exists ( $spLoadFile )) {
	$dirpath = str_ireplace ( '/plugins/bulkrankchecker/cron.php', '', $_SERVER ['SCRIPT_FILENAME'] );
	$spLoadFile = $dirpath . "/includes/sp-load.php";
...
if ($spLoadFileExist) {
	...
	if (empty($_SERVER ['REQUEST_METHOD'] )) {
		...
		$pluginInfo = $controller->__getSeoPluginInfo('bulkrankchecker', 'name' );
		$info = [];
		$info ['pid'] = $pluginInfo ['id'];
		echo $info ['action'] = "cronJob";
