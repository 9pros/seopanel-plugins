...
<?php if(!empty($saved)) showSuccessMsg($pluginText['ldsettingssaved'], false); ?>
<form id="updateSettings">
<input type="hidden" value="update" name="sec">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
	<tr class="listHead">
		<td class="left" width='30%'><?php echo $pluginText['Plugin Settings']?></td>
		<td class="right">&nbsp;</td>
	</tr>
	<?php 
	foreach( $list as $i => $listInfo){ 
		...
		<tr class="<?php echo $class?>">
			<td class="td_left_col"><?php echo $pluginText[$listInfo['set_name']]?>:</td>
			<td class="td_right_col">
				<?php if($listInfo['set_type'] != 'text'){?>
					...
					<?php }else{?>
						<?php if($listInfo['set_name'] == 'LD_SEARCH_ENGINE') {?>
							<select name="<?php echo $listInfo['set_name']?>">
								<?php
								foreach ($seList as $seInfo) {
									$selected = ($seInfo['id'] == $listInfo['set_val']) ? "selected" : "";
									?>			
				...
				<?php if($listInfo['set_name'] == 'LD_MAX_INDEXED_PAGES') {?>
					<p><?php echo $pluginText['Maximum links allowed']?>: 100</p>
				<?php }?>
			</td>
		</tr>
		<?php 
