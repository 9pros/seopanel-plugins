...
function recognizeCAPTCHASIO (
            $filename,
            $apikey,
            $is_verbose = true,
            $domain="api.captchas.io",
            $rtimeout = 5,
            $mtimeout = 120,
            $is_phrase = 0,
            $is_regsense = 0,
            ...
            )
{
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL,             "https://$domain/in.php");
    ...
    else
    {
        ...
        while(true)
        {
            $result = file_get_contents("https://$domain/res.php?key=".$apikey.'&action=get&id='.$captcha_id);
            if (strpos($result, 'ERROR')!==false)
            {
            	if ($is_verbose) echo "server returned error: $result\n";
                return false;
