...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
	<tr class="listHead">
		<td class="left" width='30%'><?=$pluginText['Plugin Settings']?></td>
		<td class="right">&nbsp;</td>
	</tr>
	<?php 
	foreach( $list as $i => $listInfo){ 
		...
		<tr class="<?=$class?>">
			...
			<td class="td_right_col">
				<?php if($listInfo['set_type'] != 'text'){?>
					...
					<?php }else{?>
						<?php if($listInfo['set_name'] == 'DI_CHECK_STATUS_DIR_TYPE_CRON') {?>
							<select name="<?=$listInfo['set_name']?>">
								<?php
								foreach ($cronchecklist as $key => $val) {
									$selected = ($key == $listInfo['set_val']) ? "selected" : "";
									?>			
		...
		<?php if ($i == 0) {?>
			<tr class="<?=$class?>">
				<td class="td_left_col" style="border-right: none;"><b><?=$pluginText['Cron Settings']?></b></td>
				<td class="td_right_col"></td>
			</tr>				
		<?php }?>
		<?php 
	}
